...
pub struct Resource {
    ...
    pub asset_contract: String, // Stellar contract ID (strkey `C...`, 56 chars) of the payment asset (e.g. a USDC SAC)
...
`asset_contract` is set at registration and is currently immutable — there is
no `set_asset_contract`. `register` always stores `DEFAULT_ASSET_CONTRACT_ID`
(the canonical **testnet** USDC SAC); `register_with_asset` lets a caller
store a different contract ID (e.g. a different network's USDC SAC, or a
different asset entirely). Distinct networks have distinct SAC addresses for
"the same" asset, so indexers use this field — not a currency label — to
identify exactly which deployment a price is denominated in.
...
| ------------------------------------------------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `register(creator, id, price, metadata, tags)`                            | `creator`                                                | `creator: Address`; `id: String` — unique cuid2 (1-24 lowercase letters/digits); `price: i128` — USDC stroops, `0 < price <= MAX_PRICE`; `metadata: String` — non-empty pointer (max 512 bytes); `tags: Vec<String>` — max 8 tags, each max 32 bytes | `Result<(), Error>`       | Register a new resource priced against `DEFAULT_ASSET_CONTRACT_ID` (testnet USDC SAC). Resources are listed by default, start `Pending` verification, and start unfrozen. Reserved IDs (`admin`, `null`, `registry`, `api`, `index`, `root`, `system`, case-insensitive) are rejected.                   |
| `register_with_asset(creator, id, price, asset_contract, metadata, tags)` | `creator`                                                | Same as `register`, plus `asset_contract: String` — a Stellar contract ID in strkey form: exactly 56 chars, starts with `C`, base32 charset (`InvalidAssetContract` otherwise; shape only, not the strkey checksum)                                  | `Result<(), Error>`       | Register a new resource priced against an explicit payment asset contract instead of the default. Otherwise identical to `register` — same validation, auth, and `register` event.                                                                                                                       |
| `set_price(id, new_price)`                                                | `creator`                                                | `id: String`; `new_price: i128` — `0 < new_price <= MAX_PRICE`                                                                                                                                                                                       | `Result<(), Error>`       | Update the resource price. Emits `setprice` with the old and new price. Does not change `asset_contract`.                                                                                                                                                                                                |
...
| `24` | `InvalidAssetContract`          | Asset contract ID is not exactly `ASSET_CONTRACT_ID_LEN` (56) chars, doesn't start with `C`, or contains characters outside the base32 strkey charset. |
...
| ----------- | -------------------------------------------------------- | ---------------------------------------------------------- |
| `register`  | `Resource` (full struct, including `asset_contract`)     | `register()` or `register_with_asset()` succeeds           |
...
whatever `asset_contract` the resource was registered with (the testnet USDC
SAC by default).
...
| `ASSET_CONTRACT_ID_LEN`     | `56`                                                         | Required length of `asset_contract` (a Stellar contract ID in strkey form).      |
| `DEFAULT_ASSET_CONTRACT_ID` | `"CBIELTK6YBZJU5UP2WWQEUCYKLPU6AUNZ2BQ4WWFEIE3USCIHMXQDAMA"` | Testnet USDC SAC `register` stores when none is given via `register_with_asset`. |
| `RESOURCE_SCHEMA_VERSION`   | `3`                                                          | Current `Resource` schema version (tags added in v2, asset_contract in v3).      |
...
### Non-breaking addition: asset contract ID (v3)

Unlike the v2 tags change, this one does not touch `register`'s signature or
behavior — it still always stores `DEFAULT_ASSET_CONTRACT_ID` (testnet USDC),
so every existing caller (server, MCP, registry-client, tests) keeps working
unchanged. Callers that want a resource priced against a different asset
contract (a different network's USDC SAC, or a different asset entirely) call
the new `register_with_asset` instead. The `Resource` struct gains an
`asset_contract` field either way, so any code deserializing `Resource`
directly (bypassing the generated bindings) needs to account for it. Note
`DEFAULT_ASSET_CONTRACT_ID` is a compile-time constant, not network-aware —
deployments to networks other than testnet must always use
`register_with_asset` with that network's real SAC address.
...
> repair, and the asset contract ID field (`register_with_asset`) described in
> this README. Redeploy from current source and update this table's Contract
> ID / Wasm Hash (plus `VAULT_REGISTRY_CONTRACT_ID` and the generated TS
> bindings via `pnpm contract:bindings`) to pick them up.
